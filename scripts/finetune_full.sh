# scripts/finetune_full.sh
#!/bin/bash
set -e

# Defaults
GPUS=${GPUS:-1}
BASE_MODEL=${BASE_MODEL:-"OpenGVLab/InternVL2_5-2B"}
TRAIN_DATA=${TRAIN_DATA:-"feedback_data/refined_v1/train.jsonl"}
OUTPUT_NAME=${OUTPUT_NAME:-"full_v1"}
EPOCHS=${EPOCHS:-1}
BATCH_SIZE=${BATCH_SIZE:-2}  # Lower for full finetune
LR=${LR:-2e-5}  # Usually lower for full finetune

# What to freeze (all false = full finetune)
FREEZE_LLM=${FREEZE_LLM:-False}
FREEZE_MLP=${FREEZE_MLP:-False}
FREEZE_BACKBONE=${FREEZE_BACKBONE:-True}  # Usually keep vision frozen
...
echo "=== Elbiat Full Finetuning ==="
echo "Base model: ${BASE_MODEL}"
echo "Train data: ${TRAIN_DATA}"
echo "Output: ${OUTPUT_DIR}"
echo "GPUs: ${GPUS}"
echo "Freeze LLM: ${FREEZE_LLM}"
echo "Freeze MLP: ${FREEZE_MLP}"
